...
DB_FILE = pathlib.Path("onboarding_db.csv")
...
def make_username(full_name: str) -> str:
    return ".".join(full_name.strip().lower().split()) if full_name.strip() else ""

def make_display_username(full_name: str) -> str:
    return ".".join(full_name.strip().split()) if full_name.strip() else ""
...
# ----------------- GUI -----------------
root = tk.Tk()
root.title("Onboarding Template Maker")
...
# Helper to build labeled entries with subtle copy button
def lfield(parent, row, label_text, show=None):
    ...
    copy_btn = ttk.Button(parent, text=' ', width=2, style='Copy.TButton',
                          command=lambda e=ent, b=None: copy_from_widget(e))
    # make copy buttons minimal and NOT in focus traversal
    copy_btn.grid(row=row, column=2, sticky='w', padx=(6,0))
    copy_btn.configure(takefocus=False)
    # small visual 'frame' using relief on a label behind button to mimic subtle icon (optional)
    return ent

# Build fields with proper tab order (entries first; copy buttons have takefocus=False)
...
# Username & Email labels with subtle copy buttons
lbl_username = ttk.Label(create_frame, text='Username: ')
lbl_username.grid(row=6, column=0, columnspan=2, sticky='w', pady=(6,0))
btn_copy_username = ttk.Button(create_frame, text=' ', width=2, style='Copy.TButton',
...
btn_copy_email = ttk.Button(create_frame, text=' ', width=2, style='Copy.TButton',
...
btns_row.columnconfigure(0, weight=3)
btns_row.columnconfigure(1, weight=4)
btns_row.columnconfigure(2, weight=3)
...
def format_email_text(rec: dict) -> str:
    display_user = make_display_username(rec.get("full_name", ""))
    text = f"""Hi,
...
def preview_copy_handler(event=None):
    try:
        ...
        # build lines and convert to HTML fragment
        lines = sel.splitlines(True)
        html_fragment = "<div>" + format_html_lines_for_fragment(lines) + "</div>"
        set_html_clipboard_fragment(html_fragment, plain_text=sel)
        # let default also happen? we handled clipboard already; prevent default to avoid double copy
        return "break"
    except Exception:
        # fallback: let default behavior copy plain text
        return
...
btn_clear.grid(row=0, column=2, sticky='we', padx=6)
...
cols = ["full_name", "username", "pc_name", "email", "ext", "ddi"]
...
def on_tree_select(event):
    ...
    if idx < 0 or idx >= len(recs):
        return
    rec = recs[idx]
    ent_full.delete(0, tk.END); ent_full.insert(0, rec.get('full_name',''))
    ent_pc.delete(0, tk.END); ent_pc.insert(0, rec.get('pc_name',''))
    ent_pass.delete(0, tk.END); ent_pass.insert(0, rec.get('password',''))
    ent_ext.delete(0, tk.END); ent_ext.insert(0, rec.get('ext',''))
    ent_ddi.delete(0, tk.END); ent_ddi.insert(0, rec.get('ddi',''))
    update_derived()
    show_preview()
...
def do_search(event=None):
    ...
    recs = load_records()
    if q:
        recs = [r for r in recs if q in r['full_name'].lower() or q in r['username'].lower() or q in r['pc_name'].lower()]
    populate_tree(recs)
